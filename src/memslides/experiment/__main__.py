...
def _cmd_profile_v6_validate(args: argparse.Namespace) -> None:
    report = validate_profile_holistic_v6_summary(Path(args.eval_dir))
    _print_json(report)


def _cmd_profile_v6_rebuild(args: argparse.Namespace) -> None:
    report = rebuild_profile_holistic_v6_summary(
        Path(args.eval_dir),
        output_dir=Path(args.output_dir) if args.output_dir else None,
        llm_ref=args.llm_ref,
    )
    _print_json(report)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    profile_v6 = sub.add_parser("profile-v6", help="Validate or rebuild V6 profile-memory summaries")
    profile_v6_sub = profile_v6.add_subparsers(dest="profile_v6_command", required=True)
    profile_v6_validate = profile_v6_sub.add_parser("validate", help="Validate a profile_holistic_v6 summary")
    profile_v6_validate.add_argument("eval_dir")
    profile_v6_validate.set_defaults(func=_cmd_profile_v6_validate)
    profile_v6_rebuild = profile_v6_sub.add_parser("rebuild", help="Rebuild a profile_holistic_v6 summary from judge records")
    profile_v6_rebuild.add_argument("eval_dir")
    profile_v6_rebuild.add_argument("--output-dir")
    profile_v6_rebuild.add_argument("--llm-ref")
    profile_v6_rebuild.set_defaults(func=_cmd_profile_v6_rebuild)
