...
class ProfileDimensionSpec:
    ...
    def dimension_ids(self) -> tuple[str, ...]:
        return (self.dimension_id, *self.aliases)


PROFILE_DIMENSIONS: tuple[ProfileDimensionSpec, ...] = (
    ProfileDimensionSpec(
        "role_decision_fit",
        "Role Decision",
        r"Role Decision $\uparrow$",
        "Role-Decision Fit",
        aliases=("content_alignment",),
    ),
    ProfileDimensionSpec(
        "narrative_priority",
        "Narrative",
    ...
    ProfileDimensionSpec(
        ...
        aliases=("visual_alignment",),
    ),
    ProfileDimensionSpec(
        "action_support",
        "Action Support",
        r"Action Support $\uparrow$",
        "Action Support",
        aliases=("profile_specificity",),
    ),
)
...
def discover_profile_summary_paths(root: Path) -> list[Path]:
    ...
    for dirpath, _dirnames, filenames in os.walk(root, onerror=lambda _error: None):
        if PROFILE_SUMMARY_FILENAME in filenames:
            paths.append(Path(dirpath) / PROFILE_SUMMARY_FILENAME)
    return sorted(paths)
...
def parse_profile_summary(summary_path: Path, payload: dict[str, Any], root: Path) -> ProfileSummaryResult:
    ...
    if not model:
        ...
        if not model:
            model = "Unspecified"
            warnings.append(f"{summary_path}: could not infer model label; using `Unspecified`.")
    ...
    by_dimension = {str(row.get("dimension_id") or ""): row for row in dimension_rows if isinstance(row, dict)}
    scores: dict[str, float | None] = {}
    probe_counts: dict[str, int] = {}
    for spec in PROFILE_DIMENSIONS:
        row = next((by_dimension.get(dimension_id) for dimension_id in spec.dimension_ids if dimension_id in by_dimension), None)
        if row is None:
...
def build_profile_summary_markdown(
    ...
    lines = [
        ...
        f"- Output: `{output_dir}`",
        "- Scope: profile-only full-deck visual V6 main-table metrics only.",
        "- Method: blind visual pairwise judging over complete round-0 decks, with three votes per comparison to reduce order and rubric wording bias.",
        ...
        "- Dimensions: Role-Decision Fit, Narrative Priority, Visual Manifestation, and Action Support.",
        "",
        "## Main Rows",
        "",
        _markdown_table(
            table_rows,
            ["Framework", "Model", "Paired probes", "Role Decision", "Narrative", "Visual", "Action Support"],
        )
        if table_rows
    ...
    lines.extend(
        [
            ...
            "## Excluded Metrics",
            "",
            "This table layer intentionally excludes PPTEval quality, SlideTailor fair eval v3, profile span/visual span, and working-memory case-study metrics.",
            "",
        ]
    )
    return "\n".join(lines)
...
def _infer_framework(path: Path) -> str:
    ...
    if "profile_generation_eval" in text:
        return "MemSlides (Ours)"
...
def _infer_model(path: Path) -> str:
    ...
    if "qwen35plus" in compact or "qwen3" in compact:
...
def _read_text(path: Path) -> str:
    try:
        ...
        if len(str(path)) < 240 or not path.drive.endswith(":"):
            raise
