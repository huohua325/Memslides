...
def _normalize_export_warnings(value: Any) -> list[str]:
    if value is None:
        return []
    if isinstance(value, str):
        ...
        if text.startswith("["):
            ...
            if isinstance(parsed, list):
                return _normalize_export_warnings(parsed)
        return [text]
...
def _latest_current_slide_mtime(workspace: Path) -> float:
    latest = 0.0
...
def _latest_current_export_mtime(workspace: Path) -> float:
    latest = 0.0
...
def _latest_export_fresh(workspace: Path) -> bool:
    newest_slide = _latest_current_slide_mtime(workspace)
    newest_export = _latest_current_export_mtime(workspace)
    return bool(newest_export and (not newest_slide or newest_export + 1.0 >= newest_slide))


def _count_current_slides(workspace: Path) -> int:
    for slide_dir in (workspace / "outputs", workspace / "slides", workspace):
        if not slide_dir.exists() or not slide_dir.is_dir():
            continue
...
def _workspace_has_outputs(workspace: Path) -> bool:
    if not workspace.exists():
        return False
    for path in workspace.rglob("*"):
        if path.is_file() and ("outputs" in path.parts or path.suffix.lower() == ".html"):
            return True
    return False


def _infer_export_status(workspace: Path, operation: dict[str, Any] | None = None) -> tuple[str, list[str]]:
    operation = operation if isinstance(operation, dict) else {}
    warnings = _normalize_export_warnings(operation.get("export_warnings"))
    intermediate = _read_json_file(workspace / "intermediate_output.json")
    if isinstance(intermediate, dict):
        warnings.extend(_normalize_export_warnings(intermediate.get("export_warnings")))
    ...
    has_outputs = _workspace_has_outputs(workspace)
    if explicit_status in {"strict", "failed", "none"}:
        if explicit_status == "none" and (has_candidate_exports or has_outputs):
            pass
        else:
    ...
    has_slides = _count_current_slides(workspace) > 0
    fresh_export = _latest_export_fresh(workspace)
...
def _operation_has_fresh_export_since_start(workspace: Path, operation: dict[str, Any] | None) -> bool:
...
def _operation_has_fresh_strict_export(workspace: Path) -> bool:
    try:
        operation = _read_json_file(workspace / OPERATION_STATE_FILENAME)
        export_status, _warnings = _infer_export_status(workspace, operation=operation)
        return bool(export_status == "strict" and _operation_has_fresh_export_since_start(workspace, operation))
    except Exception:
        return False
