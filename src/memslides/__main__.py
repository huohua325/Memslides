...
def _web(args: argparse.Namespace) -> None:
    from memslides.web.server import run_web
    ...
    run_web(
        ...
        max_active_operations=args.max_active_operations,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    web = subparsers.add_parser("web", help="Run the local single-user Web Studio")
    web.add_argument("--host", default="127.0.0.1")
    web.add_argument("--port", type=int, default=7860)
    web.add_argument("--workspace-base", help="Workspace directory for Web sessions")
    web.add_argument("--data-root", help="Data directory for local Studio state")
    web.add_argument("--log-level", choices=["summary", "info", "debug"], default="summary")
    web.add_argument("--log-file", help="Write Web server logs to this file")
    web.add_argument(
        "--max-active-operations",
        type=int,
        default=None,
        help="Maximum local Studio operations that may run at once; default is 2.",
    )
